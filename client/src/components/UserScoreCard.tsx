  ...
  return (
    <section className="w-full py-8 md:py-12 bg-card border-t border-border/50">
      <div className="container max-w-4xl mx-auto">
        ...
        {/* Score Card */}
        <div className="bg-gradient-to-br from-gray-50 to-white border border-border rounded-2xl p-6 mb-6">
              ...
              <div className="space-y-2">
                {resolvedVotes.map((vote: any) => (
                  <Link
                    ...
                    className="flex items-center gap-3 p-3 rounded-xl border border-border/50 hover:border-border hover:bg-muted/50 transition-all group"
                  >
                    {vote.isCorrect ? (
                    ...
                    <span
                      className={`text-xs font-bold px-2 py-0.5 rounded-full ${
                        vote.isCorrect
                          ...
                          : "bg-vote-a/10 text-red-500"
                      }`}
                    >
                      {vote.isCorrect ? "Acertou" : "Errou"}
                    </span>
                    <ChevronRight className="w-4 h-4 text-muted-foreground/70 group-hover:text-brand shrink-0" />
                  </Link>
                ))}
              </div>
            </div>
